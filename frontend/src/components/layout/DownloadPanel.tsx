...
function JobRow({ job, index }: { job: DownloadJob; index: number }) {
  ...
  return (
    <div className="group flex h-12 items-center gap-3 px-3 md:px-4 hover:bg-muted/40 transition-colors">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <Tooltip>
            <TooltipTrigger asChild>
              <button
                className="flex h-full items-center gap-1 text-xs text-primary/80 hover:text-primary transition-colors"
                onClick={() => {
          ...
          <button
            className="flex h-full items-center gap-0.5 text-xs text-muted-foreground hover:text-destructive transition-colors"
      ...
      {(job.status === "queued" || job.status === "downloading") && (
        <button
          className="flex h-full items-center gap-0.5 text-xs text-muted-foreground hover:text-destructive shrink-0 transition-colors"
          onClick={() => cancel.mutate(job.id)}
...
export function DownloadPanel({ onClose }: Props) {
  ...
  return (
    <div className="border-t border-border/50 flex flex-col">
      ...
      <div className="max-h-[calc(70svh-3rem)] md:max-h-56 overflow-y-auto">
        {jobs.length === 0 ? (
