...
function JobRow({ job, index }: { job: DownloadJob; index: number }) {
  ...
  return (
    <div className="group flex items-center gap-3 py-1.5 px-4 hover:bg-muted/40 transition-colors">
      ...
      {(job.status === "queued" || job.status === "downloading") && (
        <button
          className="flex items-center gap-0.5 text-xs text-muted-foreground hover:text-destructive shrink-0 transition-colors"
          onClick={() => cancel.mutate(job.id)}
...
export function DownloadPanel({ onClose }: Props) {
  ...
  return (
    <div className="border-t border-border/50 flex flex-col">
      <div className="flex justify-between items-center px-4 py-2 border-b border-border/50 shrink-0">
